# AWS Cloud Labs Portfolio

This repository documents my hands-on AWS Cloud learning journey.

I am completing beginner-friendly AWS labs to practice cloud fundamentals, AWS services, security basics, troubleshooting, and CloudOps skills.

## Goals

- Build practical AWS experience
- Document each lab clearly
- Practice AWS CLI and cloud administration
- Learn cloud security best practices
- Build a GitHub portfolio for cloud and cybersecurity roles

## Lab Progress

| # | Lab | Source Lab | Status |
|---|---|---|---|
| 1 | AWS Account & CLI Setup | AICloudFusion Lab 1A | Completed |
| 2 | Cost Budget & SNS Alert | AICloudFusion Lab 1B | Completed |
| 3 | S3 Static Website | AICloudFusion Lab 1C | Completed |
| 4 | CloudFront HTTPS Static Website | AICloudFusion Side Quest Lab 1D | Completed |
| 5 | Launch EC2 Instance | AICloudFusion Lab 2A | Completed |
| 6 | EC2 S3 IAM Role | AICloudFusion Lab 2B | Completed |
| 7 | Serverless File Pipeline | AICloudFusion Lab 2C | Completed |
| 8 | IAM Least Privilege | AICloudFusion Lab 3A | Completed |
| 9 | IAM Groups, Custom Policies & MFA | AICloudFusion Lab 3B | Completed |
| 10 | Roles, CloudTrail & Audit Logging | AICloudFusion Lab 3C | Completed |
| 11 | GuardDuty Findings | AICloudFusion Lab 4A | Completed |
| 12 | CloudTrail Investigation | AICloudFusion Lab 4B | Completed |
| 13 | Automated Security Alerts | AICloudFusion Lab 4C | Completed |
| 14 | Credential Revocation | AICloudFusion Lab 5A | Completed |
| 15 | Investigate and Report | AICloudFusion Lab 5B | Completed |
| 16 | Automated Remediation | AICloudFusion Lab 5C | Completed |
| 17 | S3 Security & Reliability | AICloudFusion Lab 6A | Completed |
| 18 | Lambda Performance & Operations | AICloudFusion Lab 6B | Completed |
| 19 | Cost Optimization & Sustainability | AICloudFusion Lab 6C | Completed |
| 20 | OpenTofu IaC Foundation | AICloudFusion Lab 7A | Completed |
| 21 | Lambda with Infrastructure as Code | AICloudFusion Lab 7B | In Progress |

## Repository Structure

```text
labs/
├── 01-aws-account-cli-setup/
├── 02-cost-budget-sns-alert/
├── 03-s3-static-website/
├── 04-cloudfront-https-static-website/
├── 05-launch-ec2-instance/
├── 06-ec2-s3-iam-role/
├── 07-serverless-file-pipeline/
├── 08-iam-least-privilege/
├── 09-iam-groups-custom-policy-mfa/
├── 10-roles-cloudtrail-audit/
├── 11-guardduty-findings/
├── 12-cloudtrail-investigation/
├── 13-automated-security-alerts/
├── 14-credential-revocation/
├── 15-investigate-and-report/
├── 16-automated-remediation/
├── 17-s3-security-reliability/
├── 18-lambda-performance-operations/
├── 19-cost-optimization-sustainability/
├── 20-opentofu-iac-foundation/
├── 21-lambda-with-iac/
└── ...