...
| 2026-08-06 | Deploy Chatbot with Structured Logging | Lab 10A | Completed | Deploying a Lambda trivia chatbot that calls an external API and writes structured JSON logs to CloudWatch for observability |
...
| 2026-08-07 | Resilient Chatbot Fallback | Lab 10C | In Progress | Adding SSM Parameter Store runtime configuration, fallback mode, and graceful degradation to keep the chatbot responsive when the external trivia API is degraded |